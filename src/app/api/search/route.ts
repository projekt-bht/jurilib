...
import { vectorizeSearch } from "@helper/vectorizer";

type SearchRequest ={
    searchID: string
}

const similarityOffset = 0.40
...
export async function POST(req:NextRequest){
    ...
    if(query){
        ...
        const matches = await prisma.$queryRawUnsafe<{ id: string; name: string; expertiseArea: string; similarity: number }[]>(`
        ...
        const filteredMatches = matches.filter((match) => match.similarity >= highestSimilarity - threshold)
