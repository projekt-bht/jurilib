...
import type { AppointmentUpdateInput, OrganizationCreateInput } from '~/generated/prisma/models';
...
const { PATCH: appointmentPATCH } =
  await import('@/app/api/appointment/employee/[employeeID]/[appointmentID]/route');
...
describe('Appointment Employee Endpoint api/appointment/employee/[employeeID]/[appointmentID] testen', () => {
  ...
  test('PATCH Appointment', async () => {
    const patchBody: AppointmentUpdateInput = {
      duration: 20,
    ...
    const res = await appointmentPATCH(patchReq, {
      params: Promise.resolve({ employeeID: cEmployee.id, appointmentID: cAppointment.id }),
    });
    ...
    expect(updated?.status).toBe(AppointmentStatus.CANCELED);
  });

  test('PATCH Appointment with invalid data', async () => {
    const postReq = new NextRequest(placeholderUrl, {
      method: 'PATCH',
      headers: {
        'content-type': 'application/json',
    ...
    const res = await appointmentPATCH(postReq, {
      params: Promise.resolve({ employeeID: cEmployee.id, appointmentID: cAppointment.id }),
    });
