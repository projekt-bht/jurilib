...
const { POST: cancelPOST } = await import('@/app/api/appointment/[appointmentID]/cancel/route');
...
describe('Appointment Employee Endpoint api/appointment/employee/[employeeID]/[appointmentID] testen', () => {
  ...
  test('Request Appointment', async () => {
    ...
    await requestPOST(postReq, {
      params: Promise.resolve({ appointmentID: cAppointment.id }),
    });
    const res = await cancelPOST(postReq, {
      params: Promise.resolve({ appointmentID: cAppointment.id }),
    });
    ...
    expect(updated?.status).toBe(AppointmentStatus.OPEN);
  });
  ...
  test('Request Appointment with invalid appointment', async () => {
    ...
    // not authorized since appointment and user doesnt match
