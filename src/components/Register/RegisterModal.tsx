...
export function RegisterModal() {
  ...
  const [userFormState, setUserFormState] = useState<FormState>(USER_FORM_STATE);
  ...
  const [userSubmissionState, setUserSubmissionState] = useState<SubmissionState>({ type: 'idle' });
  const [organizationSubmissionState, setOrganizationSubmissionState] = useState<SubmissionState>({
    type: 'idle',
  });
  ...
  const handleUserSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      resetFormState();
      setIsOpen(false);
      setSuccessMessage('Dein Benutzerkonto wurde erfolgreich erstellt! Willkommen bei JuriLib.');
  ...
  const handleOrganizationSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      resetFormState();
      setIsOpen(false);
      setSuccessMessage('Organisation und Admin-Zugang wurden erfolgreich angelegt!');
              ...
              {ORGANIZATION_TYPES.map((type) => (
                <button
                  ...
                  className={`rounded-full px-3 py-1 ${
                    organizationFormState.organizationType === type
                      ? 'bg-black text-white'
        ...
        <section>
          ...
          <div className="mt-3 grid max-h-48 gap-2 overflow-y-auto rounded-xl border border-gray-100 p-3 sm:grid-cols-2">
            {AREA_OPTIONS.map((area) => {
              const isSelected = organizationFormState.expertiseArea.includes(area);
              return (
                <label
                  key={area}
